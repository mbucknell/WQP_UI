<template>
  <div>
    <select :multiple="true" class="hidden-input" name="characteristicType">
      <option v-for="option in chargroup" :value="option" selected></option>
    </select>
    <multiselect v-model="chargroupValue" @input="updateSelected" tag-placeholder="All Characteristic Groups"
                 placeholder="All Characteristic Groups" aria-label="Input box for characteristic groups parameter"
                 label="text" track-by="id" select-label="" :max-height="200" :options="chargroupOptions"
